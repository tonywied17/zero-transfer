...
function normalizeMemoryContent(content) {
  ...
  return typeof content === "string" ? Buffer8.from(content) : new Uint8Array(content);
}
...
var HttpProvider = class {
  ...
  async connect(profile) {
    ...
    if (profile.username !== void 0) {
      ...
      headers["Authorization"] = `Basic ${Buffer9.from(`${usernameText}:${passwordText}`).toString("base64")}`;
    }
...
var WebDavProvider = class {
  ...
  async connect(profile) {
    ...
    if (profile.username !== void 0) {
      ...
      const usernameText = secretToString2(username);
      const passwordText = secretToString2(password);
      headers["Authorization"] = `Basic ${Buffer10.from(`${usernameText}:${passwordText}`).toString(
        "base64"
      )}`;
    }
...
function matchesHashedEntry(salt, hash, host, port) {
  const saltBuffer = Buffer11.from(salt, "base64");
...
function buildSiteFromFields(fields, passwordEncoding) {
  ...
  if (rawPass !== void 0 && rawPass !== "") {
    if (passwordEncoding === "base64") {
      password = Buffer12.from(rawPass, "base64").toString("utf8");
    } else {
      password = rawPass;
    }
...
function evaluateCountPolicy(entries, policy) {
  ...
  const sorted = [...files].sort((a, b) => compareEntries7(a, b, sortBy));
...
function compareEntries7(a, b, sortBy) {
  if (sortBy === "name") {
...
function signWebhookPayload(payload, secret, timestamp = (/* @__PURE__ */ new Date()).toISOString()) {
  const mac = createHmac4("sha256", secret);
