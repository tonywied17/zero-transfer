...
function normalizeBastionConfig(config) {
  ...
  if (typeof cloned.privateKey === "string") {
    cloned.privateKey = import_node_buffer5.Buffer.from(cloned.privateKey);
  }
  return cloned;
}
...
function normalizeMemoryContent(content) {
  ...
  return typeof content === "string" ? import_node_buffer8.Buffer.from(content) : new Uint8Array(content);
}
...
var HttpProvider = class {
  ...
  async connect(profile) {
    ...
    if (profile.username !== void 0) {
      ...
      headers["Authorization"] = `Basic ${import_node_buffer9.Buffer.from(`${usernameText}:${passwordText}`).toString("base64")}`;
    }
...
var WebDavProvider = class {
  ...
  async connect(profile) {
    ...
    if (profile.username !== void 0) {
      ...
      const usernameText = secretToString2(username);
      const passwordText = secretToString2(password);
      headers["Authorization"] = `Basic ${import_node_buffer10.Buffer.from(`${usernameText}:${passwordText}`).toString(
        "base64"
      )}`;
    }
...
function matchesHashedEntry(salt, hash, host, port) {
  const saltBuffer = import_node_buffer11.Buffer.from(salt, "base64");
...
function buildSiteFromFields(fields, passwordEncoding) {
  ...
  if (rawPass !== void 0 && rawPass !== "") {
    if (passwordEncoding === "base64") {
      password = import_node_buffer12.Buffer.from(rawPass, "base64").toString("utf8");
    } else {
      password = rawPass;
    }
